...
?>
<ul class="mappers-breadcrumbs">
	<li>
		<a href="<?php echo esc_url( home_url( '/' ) ); ?>">
			<?php echo $home_page_id ? esc_html( get_the_title( $home_page_id ) ) : esc_html__( 'Головна', 'mappers' ); ?>
		</a>
	</li>
	...
	if ( is_singular( 'mappers-apartment' ) || $is_apartments_page ) :
		$building_and_floor = mappers_get_building_and_floor( $target_post_id, $is_apartments_page );
		if ( $building_and_floor['building'] ) :
			?>
		<li>
			<a href="<?php echo esc_url( get_term_link( (int) $building_and_floor['building']->term_id ) ); ?>">
			<?php esc_html_e( 'Вибір корпусу', 'mappers' ); ?>
			</a>
		</li>
			<?php
			endif;
	...
	<?php
		$page_id    = null;
		$post_types = array(
			// 'mappers-apartment' => 'page-buildings.php',
		);
		foreach ( $post_types as $key => $page_template ) {
			if ( $target_post_type === $key ) {
				$page_id = mappers_get_page_id_by_template( $page_template );
				break;
			}
		}
		if ( $page_id ) :
	...
	if ( is_tax() ) :
		...
		if ( is_tax( 'mappers-building-floor' ) ) {
			$term_name = __( 'Вибір корпусу', 'mappers' );
		}
		?>
		<li>

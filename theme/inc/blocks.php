 ...
 */
function mappers_render_block_preview_in_admin( $fields, $image_slug = '' ) {
	$preview = $fields['preview'] ?? null;
	if ( ! $preview && defined( 'REST_REQUEST' ) && REST_REQUEST ) {
		if ( $image_slug ) {
			echo '<img src="' . esc_url( get_template_directory_uri() ) . '/img/blocks/' . esc_attr( $image_slug ) . '.webp" class="mappers-block-preview-img" style="display: block; max-width: 100%; height: auto; margin:0 auto;" alt="">';
		}
		return true;
	}
	return false;

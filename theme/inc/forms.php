...
add_action( 'wp_ajax_nopriv_mappers_contact_form', 'mappers_contact_form' );
add_action( 'wp_ajax_mappers_contact_form', 'mappers_contact_form' );
 ...
 * @return void
 */
function mappers_contact_form() {

	check_ajax_referer( 'mappers_ajaxnonce', 'nonce' );

	$fields = mappers_check_ajax_text_fields(
		array(
			'name',
			'email',
			'tel',
	...
	$titles = array(
		'name'  => __( 'Name', 'mappers' ),
		'email' => __( 'Email', 'mappers' ),
		'tel'   => __( 'Phone', 'mappers' ),
		'text'  => __( 'Message', 'mappers' ),
	);
	...
	if ( ! $fields['subject'] ) {
		$fields['subject'] = esc_html( __( 'Нова заявка', 'mappers' ) );
	}

	$msg = '<p><b>' . esc_html__( 'Дата', 'mappers' ) . ':</b> ' . wp_date( 'Y-m-d H:i:s' ) . '</p>';
	...
	if ( $fields['user_id'] ) {
		$msg .= '<p><b>' . esc_html__( 'User', 'mappers' ) . ': </b> <a href="' . esc_url( admin_url( 'user-edit.php?user_id=' . $fields['user_id'] ) ) . '">' . esc_html( $fields['user_id'] ) . '</a></p>';
	}
	if ( $fields['post_type'] && $fields['post_id'] ) {
		$msg .= '<p><b>' . esc_html__( 'Post', 'mappers' ) . ': </b> <a href="' . esc_url( get_edit_post_link( $fields['post_id'] ) ) . '">' . esc_html( $fields['post_id'] ) . '</a></p>';
	}
	if ( $file_links ) {
		$msg .= '<p><b>' . esc_html__( 'Files', 'mappers' ) . ': </b> ' . $file_links . '</p>';
	}
	...
	wp_send_json_error( array( 'message' => esc_html__( 'Error submitting form.', 'mappers' ) ) );
}

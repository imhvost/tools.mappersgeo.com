...
class Container_Condition_Provider implements ServiceProviderInterface {
	...
	protected function install_conditions( $ioc ) {
		...
		$ioc['container_condition_fulfillable_collection'] = $ioc->factory(
			function ( $ioc ) {
		...
		$cc_ioc['post_parent_id']   = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['post_ancestor_id'] = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['post_type']        = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['post_format']      = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['post_level']       = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['post_template']    = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['post_term']        = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['term_taxonomy'] = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['term_level']    = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['term_parent']   = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['term_ancestor'] = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['user_role']       = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['user_capability'] = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['current_user_id']         = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['current_user_role']       = $cc_ioc->factory(
			function () use ( $ioc ) {
		...
		$cc_ioc['current_user_capability'] = $cc_ioc->factory(
			function () use ( $ioc ) {
	...
	protected function install_container_conditions( $ioc ) {
		...
		add_filter(
			'carbon_fields_container_static_condition_types',
			function ( $condition_types, $container_type, $container ) {
				return array_merge(
					$condition_types,
